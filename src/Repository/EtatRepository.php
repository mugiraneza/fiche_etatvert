...
class EtatRepository extends ServiceEntityRepository
{
   ...
   /* public function afficheEtatVert(int $dossierId)
    {

        return $this->createQueryBuilder("et")
            ->join("Sousdossier","ss",Join::WITH ,"et.id=ss.etats")
            ->join("Personnel","pp",Join::WITH,"pp.id=ss.personnel")
            ->andWhere("et.moistraitement=:periodicite")
            ->where("ss.id=:dossierId")
            ->setParameter("periodicite",16)
            ->setParameter("dossierId",$dossierId)->getQuery()->getResult();
    }*/
